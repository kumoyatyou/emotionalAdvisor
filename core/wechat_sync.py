    ...
    async def fetch_latest_messages(self, contact_id: str, limit: int = 100) -> str:
        ...
        try:
            ...
            async with httpx.AsyncClient(trust_env=False, timeout=60.0) as client:
                ...
                if last_timestamp > 0:
                    payload["start"] = str(last_timestamp)
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success") and "messages" in data:
                        ...
                        if not new_messages:
                            time_str = datetime.fromtimestamp(last_timestamp).strftime("%Y-%m-%d %H:%M:%S") if last_timestamp > 0 else "未知时间"
                            return f"与 '{contact_id}' 的聊天记录已是最新，没有发现新消息。 (最新记录时间: {time_str})"
                            
                        # 按时间升序排列
                        new_messages.sort(key=lambda x: x.get("createTime", 0))
                        
                        existing_ids = {str(m.get("localId")) for m in data_to_write["messages"]}
                        added_count = 0
                        ...
                        for msg in new_messages:
                            ...
                            if str(msg_obj["localId"]) not in existing_ids:
                                data_to_write["messages"].append(msg_obj)
                                kb_messages.append(msg_obj)
                                existing_ids.add(str(msg_obj["localId"]))
                                added_count += 1
                                
                        if added_count == 0:
                            time_str = datetime.fromtimestamp(last_timestamp).strftime("%Y-%m-%d %H:%M:%S") if last_timestamp > 0 else "未知时间"
                            return f"与 '{contact_id}' 的聊天记录已是最新，没有新增内容。 (最新记录时间: {time_str})"
